...
@Slf4j
@Service
public class HgAgBallServiceImpl implements IHgAgBallService {
    ...
    // AG固定返水比例
    private final static double rebateAg = 0.018;
    ...
    public void betCheck012(BetParamVo betParamVo) {
        ...
        if (zong0_1!=0 && zong2_3!=0 && zong4_6!=0 && zong7!=0) {
            *//** AG0123，皇冠 1）总进球4-6,  2）总进球7+ *//*
            log.info("      @@足球, AG 0123, 皇冠 ①总进球4-6,  ②总进球7+ ------------------------------------------------------");
            SP0123_HGZong4_6_7(betParamVo);
            log.info("");

            *//** AG01456，皇冠 1）总进球2-3,  2）总进球7+ *//*
            log.info("      @@足球, AG 01456, 皇冠 ①总进球2-3,  ②总进球7+ ------------------------------------------------------");
            SP01456_HGZong2_3_7(betParamVo);
            log.info("");

            *//** AG017+，皇冠 1）总进球2-3,  2）总进球4-6 *//*
            log.info("      @@足球, AG 017+, 皇冠 ①总进球2-3,  ②总进球4-6 ------------------------------------------------------");
            SP017_HGZong2_3_4_6(betParamVo);
            log.info("");

            *//** AG4567+，皇冠 1）总进球0-1,  2）总进球2-3 *//*
            log.info("      @@足球, AG 4567+, 皇冠 ①总进球0-1,  ②总进球2-3 ------------------------------------------------------");
    ...
    public void betCheckSingle(BetParamVo betParamVo) {
        ...
        if (home!=0 && tie!=0 && visit!=0) {
            log.info("       @@足球, AG @主队胜, 皇冠 @和局 @客胜 -----------------------------");
            SPWin_HGTieAndLose(betParamVo);
            log.info("");
            *//** AG平，皇冠（主队胜、客队胜） *//*
            log.info("       @@足球, AG @平, 皇冠 @主胜 @客胜 -----------------------------");
            SPTie_HGWinAndLose(betParamVo);
            log.info("");
            *//** AG主队负，皇冠（和局、主队胜） *//*
            log.info("       @@足球, AG @主负, 皇冠 @平 @主胜 -----------------------------");
...
//        Double oddsRangWin = betParamVo.getOddsRangWin();
    ...
    public void SPWin_HGTieAndLose(BetParamVo betParamVo) {
        ...
        // AG返水
        Double rebateSpAmount = CalcUtil.mul(betAmountSp, rebateAg);

        /** AG中奖收益:AG奖金 - 皇冠本金 + AG返水 + 皇冠返水 */
        // AG奖金
        ...
        /** 皇冠和局收益:皇冠和局奖金 - 皇冠客队胜本金 - AG本金 + AG返水 + 皇冠返水 */
        // 皇冠奖金
        ...
        /** 皇冠客胜收益:皇冠客胜奖金 - 皇冠和局本金 - AG本金 + AG返水 + 皇冠返水 */
        // 皇冠奖金
        ...
        if(rewardSp > 0 && rewardHgTie > 0 && rewardHgVisit > 0) {
            log.info("调整后 AG投注：主胜 @" + oddsWin + ", 投 " + betAmountSp.intValue() + ", 收益：" + rewardSp              + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, betAmountSp, 4), 100) + "％");
            ...
            String msg = betParamVo.getMsg() + ",  AG投注：主 胜 @" + oddsWin + ", 金额 " + betAmountSp.intValue()
    ...
    public void SPTie_HGWinAndLose(BetParamVo betParamVo) {
        ...
        /** 皇冠客胜收益:皇冠客胜奖金 - 皇冠主胜本金 - AG本金 + AG返水 + 皇冠返水 */
        // 皇冠奖金
        Double bonusHgVisit = CalcUtil.mul(CalcUtil.sub(oddsHgVisit, 1), betAmountHgVisit);
        // 皇冠奖金返水 = 客胜奖金 + 和局本金
        ...
        if(rewardSp > 0 && rewardHgHome > 0 && rewardHgVisit > 0){
            log.info(" AG投注：和局 @" + oddsTie + ", 投 " + betAmountSp.intValue() + ", 收益：" + rewardSp              + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, betAmountSp, 4), 100) + "％");
            ...
            String msg = betParamVo.getMsg() + ",  AG投注：平 @" + oddsTie + ", 金额 " + betAmountSp.intValue()
    ...
    public void SPLose_HGWinAndTie(BetParamVo betParamVo) {
        ...
        /** 皇冠主胜收益:皇冠客胜奖金 - 皇冠主胜本金 - AG本金 + AG返水 + 皇冠返水 */
        // 皇冠奖金
        Double bonusHgHome = CalcUtil.mul(CalcUtil.sub(oddsHgHome, 1), betAmountHgHome);
        // 皇冠奖金返水 = 客胜奖金 + 和局本金
    ...
    private void revision(BetParamVo betParamVo, Double oddsLose, Double betAmountSp, Double oddsHgTie, Double oddsHgHome, Double betAmountHgTie, Double betAmountHgHome, Double rebateSpAmount, Double bonusSp, Double rewardSp, Double rewardHgTie, Double rewardHgHome) {
        ...
        if(rewardSp > 0 && rewardHgTie > 0 && rewardHgHome > 0){
            log.info(" AG投注：主负 @" + oddsLose + ", 投 " + betAmountSp.intValue() + ", 收益：" + rewardSp.intValue()  + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, betAmountSp, 4), 100) + "％");
            ...
            String msg = betParamVo.getMsg() + ",  AG投注：客胜 @" + oddsLose + ", 金额 " + betAmountSp.intValue()
                    + ",  皇冠投注：① 和局 @" + oddsHgTie + ", 金额 " + betAmountHgTie.intValue()
    ...
    public BetParamVo SPWin_HGVisitAdd05(BetParamVo betParamVo) {
        ...
        log.info("AG投注：主 胜 @" + oddsWin + ", 投 " + betAmountSp.intValue());
    ...
    public void SPRangWin_HGVisitAdd05(BetParamVo betParamVo) {
        ...
        log.info("AG投注：主 让球胜 @" + oddsWin + ", 投 " + betAmountSp.intValue());
        log.info("皇冠投注：客 +05 @" + oddsHg + ", 投" + betAmountHg.intValue());
        log.info("收益：" + rewardSp.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, CalcUtil.add(betAmountSp, betAmountHg), 4), 100) + "％");
    }
    ...
    public BetParamVo SPShouWin_HGVisitCut05(BetParamVo betParamVo) {
        ...
        log.info("AG投注：主 受球胜 @" + oddsWin + ", 投 " + betAmountSp.intValue());
        log.info("皇冠投注：客 -05 @" + oddsHg + ", 投" + betAmountHg.intValue());
        log.info("收益：" + rewardSp.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, CalcUtil.add(betAmountSp, betAmountHg), 4), 100) + "％");

        if (betParamVo.getNotifyFlag() == 1) {
            String msg = betParamVo.getMsg() + ",  AG投注：主 受球胜 @" + oddsWin + ", 金额 " + betAmountSp.intValue()
                    + ",  皇冠投注：客 -05 @" + oddsHg + ", 金额 " + betAmountHg.intValue()
                    + ",  收益：" + rewardSp.intValue() + ",  收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, CalcUtil.add(betAmountSp, betAmountHg), 4), 100) + "％";
    ...
    public BetParamVo SPLose_HGHomeAdd05(BetParamVo betParamVo) {
        ...
        log.info("AG投注：主 负 @" + oddsLose + ", 投 " + betAmountSp.intValue());
        log.info("皇冠投注：主 +05 @" + oddsHg + ", 投" + betAmountHg.intValue());
        log.info("收益：" + rewardSp.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, CalcUtil.add(betAmountSp, betAmountHg), 4), 100) + "％");

        if (betParamVo.getNotifyFlag() == 1) {
            String msg = betParamVo.getMsg() + ",  AG投注：主 负 @" + oddsLose + ", 金额 " + betAmountSp.intValue()
    ...
    public BetParamVo SPRangLose_HGHomeCut05(BetParamVo betParamVo) {
        ...
        log.info("AG投注：主 让球负 @" + oddsLose + ", 投 " + betAmountSp.intValue());
        log.info("皇冠投注：主 -05 @" + oddsHg + ", 投" + betAmountHg.intValue());
        log.info("收益：" + rewardSp.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, CalcUtil.add(betAmountSp, betAmountHg), 4), 100) + "％");

        if (betParamVo.getNotifyFlag() == 1) {
            String msg = betParamVo.getMsg() + ",  AG投注：主 让球负 @" + oddsLose + ", 金额 " + betAmountSp.intValue()
                    + ",  皇冠投注：主 -05 @" + oddsHg + ", 金额 " + betAmountHg.intValue()
                    + ",  收益：" + rewardSp.intValue() + ",  收益率：" + CalcUtil.mul(CalcUtil.div(rewardSp, CalcUtil.add(betAmountSp, betAmountHg), 4), 100) + "％";
    ...
    public void SP01_HG大15(BetParamVo betParamVo) {
        ...
        // AG返水
        double rebateSPAmount = CalcUtil.mul(CalcUtil.add(betAmountZero, betAmountOne), rebateAg);

        log.info("AG投注：0球 " + betAmountZero.intValue() + ", 1球 " + betAmountOne.intValue());
        log.info("皇冠投注：大1.5 @" + oddsHg + ", " + betAmountHg.intValue());
        log.info("AG总投注：" + CalcUtil.add(betAmountZero, betAmountOne).intValue() + ", 皇冠总投注：" + betAmountHg.intValue());
        log.info("");
    ...
    public void SP012_HG大2(BetParamVo betParamVo) {
        ...
        // AG返水
        double rebateSPAmount = CalcUtil.mul(CalcUtil.add(betAmountZero, betAmountOne, betAmountTwo), rebateAg);

        log.info("AG投注：0球 " + betAmountZero.intValue() + ", 1球 " + betAmountOne.intValue() + ", 2球 " + betAmountTwo.intValue());
        log.info("皇冠投注：大2 @" + oddsHg + ", " + betAmountHg.intValue());
        log.info("AG总投注：" + CalcUtil.add(betAmountZero, betAmountOne, betAmountTwo).intValue() + ", 皇冠总投注：" + betAmountHg.intValue());
        log.info("");
    ...
    public void SP012_HG大25(BetParamVo betParamVo) {
        ...
        log.info("AG总投注：" + CalcUtil.add(betAmountZero, betAmountOne, betAmountTwo).intValue()
    ...
    public void SP012_HG大3(BetParamVo betParamVo) {
        ...
        double oddsHg = betParamVo.getOddsHg();
        // AG返水
        double rebateSPAmount = CalcUtil.mul(CalcUtil.add(betAmountZero, betAmountOne, betAmountTwo, betAmountThree), rebateAg);

        log.info("AG投注：0球 " + betAmountZero.intValue() + ", 1球 " + betAmountOne.intValue() + ", 2球 " + betAmountTwo.intValue() + ", 3球 " + betAmountThree.intValue());
        log.info("皇冠投注：大3 @" + oddsHg + ", " + betAmountHg.intValue());
        log.info("AG总投注：" + CalcUtil.add(betAmountZero, betAmountOne, betAmountTwo, betAmountThree).intValue()
                + ", 皇冠总投注：" + betAmountHg.intValue());
        log.info("");
    ...
    /**
     * 大3/3.5
     * AG小 0123,皇冠 大3/3.5 3球AG赢，皇冠输一半
     * @param betParamVo
     */
    ...
    /**
     * 大2.5/3
     * AG小 0123,皇冠 大2.5/3, 3球AG赢，皇冠赢一半
     * @param betParamVo
     */
    ...
    /**
     * 小3.5
     * AG大 456+,皇冠 小3.5 全输全赢
     * @param betParamVo
     */
    public void SP456_HG小35(BetParamVo betParamVo) {
        Double betAmountHg = betParamVo.getBetAmountHg();
        betParamVo.setBetBaseAmount(betAmountHg);
        // 计算AG初始投注金额
        BetParamVo betParamTemp = AdaptationAmount.adaptation(betParamVo);
        ...
        double oddsHg = betParamVo.getOddsHg();
        // AG返水
        double rebateSPAmount = CalcUtil.mul(CalcUtil.add(betAmountFour, betAmountFive, betAmountSix), rebateAg);

        log.info("AG投注：4球 " + betAmountFour.intValue() + ", 5球 " + betAmountFive.intValue() + ", 6球 " + betAmountSix.intValue());
        log.info("皇冠投注：小3.5 @" + oddsHg + ", " + betAmountHg.intValue());
        log.info("AG总投注：" + CalcUtil.add(betAmountFour, betAmountFive, betAmountSix).intValue()
                + ", 皇冠总投注：" + betAmountHg.intValue());
        log.info("");

        Double betAmountAll = CalcUtil.add(betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        ...
        Double reward4 = CalcUtil.sub(CalcUtil.add(bonusFour, rebateSPAmount, rebateHGAmount), betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        ...
        Double reward5 = CalcUtil.sub(CalcUtil.add(bonusFive, rebateSPAmount, rebateHGAmount), betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        ...
        Double reward6 = CalcUtil.sub(CalcUtil.add(bonusSix, rebateSPAmount, rebateHGAmount), betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        ...
        Double rewardHg = CalcUtil.sub(CalcUtil.add(bonusHg, rebateSPAmount, rebateHGAmount1), betAmountFour, betAmountFive, betAmountSix);
        betParamVo.setRewardHG(rewardHg);
        log.info("皇冠收益：" + rewardHg.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardHg, betAmountAll, 4), 1000) + "‰");
    }
    ...
    /**
     * 小2.5
     * AG大 34567+,皇冠 小2.5 全输全赢
     * @param betParamVo
     */
    public void SP3456_HG小25(BetParamVo betParamVo) {
        Double betAmountHg = betParamVo.getBetAmountHg();
        betParamVo.setBetBaseAmount(betAmountHg);
        // 计算AG初始投注金额
        BetParamVo betParamTemp = AdaptationAmount.adaptation(betParamVo);
        ...
        log.info("AG投注：3球 " + betAmountThree.intValue() + ", 4球 " + betAmountFour.intValue() + ", 5球 " + betAmountFive.intValue() + ", 6球 " + betAmountSix.intValue());
        log.info("皇冠投注：小2.5 @" + oddsHg + ", " + betAmountHg.intValue());
        log.info("AG总投注：" + CalcUtil.add(betAmountThree, betAmountFour, betAmountFive, betAmountSix).intValue()
                + ", 皇冠总投注：" + betAmountHg.intValue());
        log.info("");

        Double betAmountAll = CalcUtil.add(betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        ...
        /** 3球数据 */
        double bonusThree = CalcUtil.mul(betAmountThree, oddsThree); // 奖金
        Double reward3 = CalcUtil.sub(CalcUtil.add(bonusThree, rebateSPAmount, rebateHGAmount), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward4(reward3);
        log.info("3球收益：" + reward3.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward3, betAmountAll, 4), 1000) + "‰");

        /** 4球数据 */
        double bonusFour = CalcUtil.mul(betAmountFour, oddsFour); // 奖金
        Double reward4 = CalcUtil.sub(CalcUtil.add(bonusFour, rebateSPAmount, rebateHGAmount), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward4(reward4);
        log.info("4球收益：" + reward4.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward4, betAmountAll, 4), 1000) + "‰");

        /** 5球数据 */
        double bonusFive = CalcUtil.mul(betAmountFive, oddsFive); // 奖金
        Double reward5 = CalcUtil.sub(CalcUtil.add(bonusFive, rebateSPAmount, rebateHGAmount), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward5(reward5);
        log.info("5球收益：" + reward5.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward5, betAmountAll, 4), 1000) + "‰");

        /** 6球数据 */
        double bonusSix = CalcUtil.mul(betAmountSix, oddsSix); // 奖金
        Double reward6 = CalcUtil.sub(CalcUtil.add(bonusSix, rebateSPAmount, rebateHGAmount), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward6(reward6);
        log.info("6球收益：" + reward6.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward6, betAmountAll, 4), 1000) + "‰");
        ...
        Double rewardHg = CalcUtil.sub(CalcUtil.add(bonusHg, rebateSPAmount, rebateHGAmount1), betAmountThree, betAmountFour, betAmountFive, betAmountSix);
        betParamVo.setRewardHG(rewardHg);
        log.info("皇冠收益：" + rewardHg.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardHg, betAmountAll, 4), 1000) + "‰");
    }
    ...
    /**
     * 小2/2.5
     * AG大 234567+,皇冠 小2/2.5, 2球AG赢，皇冠输一半
     * @param betParamVo
     */
    public void SP23456_HG小2_25(BetParamVo betParamVo) {
        Double betAmountHg = betParamVo.getBetAmountHg();
        betParamVo.setBetBaseAmount(betAmountHg);
        // 计算AG初始投注金额
        BetParamVo betParamTemp = AdaptationAmount.adaptation(betParamVo);
        ...
        log.info("AG投注：2球 " + betAmountTwo.intValue()+ ", 3球 " + betAmountThree.intValue() + ", 4球 " + betAmountFour.intValue() + ", 5球 " + betAmountFive.intValue() + ", 6球 " + betAmountSix.intValue());
        log.info("皇冠投注：小2/2.5 @" + oddsHg + ", " + betAmountHg.intValue());
        log.info("AG总投注：" + CalcUtil.add(betAmountTwo, betAmountThree, betAmountFour, betAmountFive, betAmountSix).intValue()
                + ", 皇冠总投注：" + betAmountHg.intValue());
        log.info("");
        ...
        /** 2球数据 */
        double bonusTwo = CalcUtil.mul(betAmountTwo, oddsTwo); // 奖金
        Double reward2 = CalcUtil.sub(CalcUtil.add(bonusTwo, rebateSPAmount, rebateHGAmountHalf), betAmountTwo, betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHgHalf);
        betParamVo.setReward3(reward2);
        log.info("2球收益：" + reward2.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward2, betAmountAll, 4), 1000) + "‰");

        /** 3球数据 */
        double bonusThree = CalcUtil.mul(betAmountThree, oddsThree); // 奖金
        Double reward3 = CalcUtil.sub(CalcUtil.add(bonusThree, rebateSPAmount, rebateHGAmountHalf), betAmountTwo, betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHgHalf);
        betParamVo.setReward3(reward3);
        log.info("3球收益：" + reward3.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward3, betAmountAll, 4), 1000) + "‰");

        /** 4球数据 */
        double bonusFour = CalcUtil.mul(betAmountFour, oddsFour); // 奖金
        Double reward4 = CalcUtil.sub(CalcUtil.add(bonusFour, rebateSPAmount, rebateHGAmountAll), betAmountTwo, betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward4(reward4);
        log.info("4球收益：" + reward4.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward4, betAmountAll, 4), 1000) + "‰");

        /** 5球数据 */
        double bonusFive = CalcUtil.mul(betAmountFive, oddsFive); // 奖金
        Double reward5 = CalcUtil.sub(CalcUtil.add(bonusFive, rebateSPAmount, rebateHGAmountAll), betAmountTwo, betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward5(reward5);
        log.info("5球收益：" + reward5.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward5, betAmountAll, 4), 1000) + "‰");

        /** 6球数据 */
        double bonusSix = CalcUtil.mul(betAmountSix, oddsSix); // 奖金
        Double reward6 = CalcUtil.sub(CalcUtil.add(bonusSix, rebateSPAmount, rebateHGAmountAll), betAmountTwo, betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward6(reward6);
        log.info("6球收益：" + reward6.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward6, betAmountAll, 4), 1000) + "‰");
        ...
        Double rewardHg = CalcUtil.sub(CalcUtil.add(bonusHg, rebateSPAmount, rebateHGAmount), betAmountTwo, betAmountThree, betAmountFour, betAmountFive, betAmountSix);
        betParamVo.setRewardHG(rewardHg);
        log.info("皇冠收益：" + rewardHg.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardHg, betAmountAll, 4), 1000) + "‰");
    }
    ...
    /**
     * 小2.5/3
     * AG大 3456+,皇冠 小2.5/3, 3球AG赢，皇冠输一半
     * @param betParamVo
     */
    public void SP3456_HG小25_3(BetParamVo betParamVo) {
        Double betAmountHg = betParamVo.getBetAmountHg();
        betParamVo.setBetBaseAmount(betAmountHg);
        // 计算AG初始投注金额
        BetParamVo betParamTemp = AdaptationAmount.adaptation(betParamVo);
        ...
        /** 3球数据 */
        double bonusThree = CalcUtil.mul(betAmountThree, oddsThree); // 奖金
        Double reward3 = CalcUtil.sub(CalcUtil.add(bonusThree, rebateSPAmount, rebateHGAmountHalf), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHgHalf);
        betParamVo.setReward3(reward3);
        log.info("3球收益：" + reward3.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward3, betAmountAll, 4), 1000) + "‰");

        /** 4球数据 */
        double bonusFour = CalcUtil.mul(betAmountFour, oddsFour); // 奖金
        Double reward4 = CalcUtil.sub(CalcUtil.add(bonusFour, rebateSPAmount, rebateHGAmountAll), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward4(reward4);
        log.info("4球收益：" + reward4.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward4, betAmountAll, 4), 1000) + "‰");

        /** 5球数据 */
        double bonusFive = CalcUtil.mul(betAmountFive, oddsFive); // 奖金
        Double reward5 = CalcUtil.sub(CalcUtil.add(bonusFive, rebateSPAmount, rebateHGAmountAll), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward5(reward5);
        log.info("5球收益：" + reward5.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward5, betAmountAll, 4), 1000) + "‰");

        /** 6球数据 */
        double bonusSix = CalcUtil.mul(betAmountSix, oddsSix); // 奖金
        Double reward6 = CalcUtil.sub(CalcUtil.add(bonusSix, rebateSPAmount, rebateHGAmountAll), betAmountThree, betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward6(reward6);
        log.info("6球收益：" + reward6.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward6, betAmountAll, 4), 1000) + "‰");
        ...
        Double rewardHg = CalcUtil.sub(CalcUtil.add(bonusHg, rebateSPAmount, rebateHGAmount), betAmountThree, betAmountFour, betAmountFive, betAmountSix);
        betParamVo.setRewardHG(rewardHg);
        log.info("皇冠收益：" + rewardHg.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardHg, betAmountAll, 4), 1000) + "‰");
    }
    ...
    /**
     * 小3/3.5
     * AG大 3456+,皇冠 小3/3.5, 3球AG赢，皇冠赢一半
     * @param betParamVo
     */
    public void SP3456_HG小3_35(BetParamVo betParamVo) {
        Double betAmountHg = betParamVo.getBetAmountHg();
        betParamVo.setBetBaseAmount(betAmountHg);
        // 计算AG初始投注金额
        BetParamVo betParamTemp = AdaptationAmount.adaptation(betParamVo);
        ...
        /** 3球数据 */
        double bonusThree = CalcUtil.mul(betAmountThree, oddsThree); // 奖金
        Double reward3 = CalcUtil.sub(CalcUtil.add(bonusThree, rebateSPAmount, rebateHGAmountHalf, bonusHgHalf), betAmountThree, betAmountFour, betAmountFive, betAmountSix);
        betParamVo.setReward3(reward3);
        log.info("3球收益：" + reward3.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward3, betAmountAll, 4), 1000) + "‰");
    ...
    /**
     * 小3.5/4
     * AG大 456+,皇冠 小3.5/4, 4球AG赢，皇冠输一半
     * @param betParamVo
     */
    public void SP456_HG小35_4(BetParamVo betParamVo) {
        Double betAmountHg = betParamVo.getBetAmountHg();
        betParamVo.setBetBaseAmount(betAmountHg);
        // 计算AG初始投注金额
        BetParamVo betParamTemp = AdaptationAmount.adaptation(betParamVo);
        ...
        /** 4球数据 */
        double bonusFour = CalcUtil.mul(betAmountFour, oddsFour); // 奖金
        Double reward4 = CalcUtil.sub(CalcUtil.add(bonusFour, rebateSPAmount, rebateHGAmountHalf), betAmountFour, betAmountFive, betAmountSix, betAmountHgHalf);
        betParamVo.setReward4(reward4);
        log.info("4球收益：" + reward4.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward4, betAmountAll, 4), 1000) + "‰");

        /** 5球数据 */
        double bonusFive = CalcUtil.mul(betAmountFive, oddsFive); // 奖金
        Double reward5 = CalcUtil.sub(CalcUtil.add(bonusFive, rebateSPAmount, rebateHGAmountAll), betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward5(reward5);
        log.info("5球收益：" + reward5.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward5, betAmountAll, 4), 1000) + "‰");

        /** 6球数据 */
        double bonusSix = CalcUtil.mul(betAmountSix, oddsSix); // 奖金
        Double reward6 = CalcUtil.sub(CalcUtil.add(bonusSix, rebateSPAmount, rebateHGAmountAll), betAmountFour, betAmountFive, betAmountSix, betAmountHg);
        betParamVo.setReward6(reward6);
        log.info("6球收益：" + reward6.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward6, betAmountAll, 4), 1000) + "‰");
        ...
        Double rewardHg = CalcUtil.sub(CalcUtil.add(bonusHg, rebateSPAmount, rebateHGAmount), betAmountFour, betAmountFive, betAmountSix);
        betParamVo.setRewardHG(rewardHg);
        log.info("皇冠收益：" + rewardHg.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(rewardHg, betAmountAll, 4), 1000) + "‰");
    }
    ...
    public void SP01456_HGZong2_3_7(BetParamVo betParamVo) {
        ...
        betParamTemp.setZong7(zong7);
        // AG返水
        double rebateSPAmount = CalcUtil.mul(CalcUtil.add(betAmountZero, betAmountOne, betAmountFour, betAmountFive, betAmountSix), rebateAg);
        ...
        log.info("皇冠总进球7+收益：" + reward7.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward7, betAmountAll, 4), 1000) + "‰");
        log.info("");
        log.info("AG投注：0球 " + betAmountZero.intValue() + ", 1球 " + betAmountOne.intValue() + ", 4球 " + betAmountFour.intValue() + ", 5球 " + betAmountFive.intValue() + ", 6球 " + betAmountSix.intValue());
        log.info("皇冠投注：总进球 ①2-3@" + zong2_3 + "  ￥" + zong2_3Amount.intValue() + ",  ②7+@" + zong7 + "  ￥" + zong7Amount);
        log.info("AG总投注：" + CalcUtil.add(betAmountZero, betAmountOne, betAmountFour, betAmountFive, betAmountSix).intValue()
    ...
    public void SP017_HGZong2_3_4_6(BetParamVo betParamVo) {
        ...
        betParamTemp.setZong4_6(zong4_6);
        // AG返水
        double rebateSPAmount = CalcUtil.mul(CalcUtil.add(betAmountZero, betAmountOne, betAmountSeven), rebateAg);
        ...
        log.info("皇冠总进球4-6收益：" + reward4_6.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward4_6, betAmountAll, 4), 1000) + "‰");
        log.info("");
        log.info("AG投注：0球 " + betAmountZero.intValue() + ", 1球 " + betAmountOne.intValue() + ", 7球 " + betAmountSeven.intValue());
        log.info("皇冠投注：总进球 ①2-3@" + zong2_3 + "  ￥" + zong2_3Amount.intValue() + ",  ②4-6@" + zong4_6 + "  ￥" + zong4_6Amount);
        log.info("AG总投注：" + CalcUtil.add(betAmountZero, betAmountOne, betAmountSeven).intValue()
    ...
    public void SP4567_HGZong0_1_2_3(BetParamVo betParamVo) {
        ...
        betParamTemp.setZong2_3(zong2_3);
        // AG返水
        double rebateSPAmount = CalcUtil.mul(CalcUtil.add(betAmountFour, betAmountFive, betAmountSix, betAmountSeven), rebateAg);
        ...
        log.info("皇冠总进球2-3收益：" + reward2_3.intValue() + ", 收益率：" + CalcUtil.mul(CalcUtil.div(reward2_3, betAmountAll, 4), 1000) + "‰");
        log.info("");
        log.info("AG投注：4球 " + betAmountFour.intValue() + ", 5球 " + betAmountFive.intValue() + ", 6球 " + betAmountSix.intValue() + ", 7+球 " + betAmountSeven.intValue());
        log.info("皇冠投注：总进球 ①0-1@" + zong0_1 + "  ￥" + zong0_1Amount.intValue() + ",  ②2-3@" + zong2_3 + "  ￥" + zong2_3Amount);
        log.info("AG总投注：" + CalcUtil.add(betAmountFour, betAmountFive, betAmountSix, betAmountSeven).intValue()
   ...
   public void betBasketball(BetBasketballParamVo basketballParamVo){
       ...
       if (oddsCutWin != 0 && visitAdd != 0) {
           log.info("       @@篮球@让分胜负, AG @主减胜, 皇冠 @客加胜 ------------------------------------------------------");
       ...
       if (oddsAddWin != 0 && visitCut != 0) {
           log.info("       @@篮球@让分胜负, AG @主加胜, 皇冠 @客减胜 ------------------------------------------------------");
    ...
    private void BK_winLose_rangFen(Double betAmountSp, Double oddsSp, Double oddsHg, Integer flag) {
        ...
        log.info("  AG投注 @"+ oddsSp +", "+betAmountSp.intValue() +", 收益:"+ rewardSp.intValue() +", 收益率:"+ CalcUtil.mul(CalcUtil.div(rewardSp, CalcUtil.add(betAmountSp, betAmountHg), 4), 100) + "%");
     ...
     * @param msg
     */
    private void nofifyMsg(Long spId, String msg, BetTypeEnum betTypeEnum) {
        NotifyMsg msgByCondition = notifyMsgMapper.findMsgByCondition(betTypeEnum.getValue(), spId, HandicapEnum.sp.getValue());
        ...
        if (msgByCondition == null || currentTime - msgByCondition.getNotifyTime() > 600000) {
            ...
            notifyMsg.setHandicap(HandicapEnum.sp.getValue());
