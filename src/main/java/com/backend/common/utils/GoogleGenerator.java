...
public class GoogleGenerator {
    ...
    // 生成的key长度( Generate secret key length) / 谷歌秘钥长度固定10位
    public static final int SECRET_SIZE = 10;

    public static final int PWD_SIZE = 5;

    public static final String SEED = "22150146801713967E8g_Backend+!1@2#3$4%5^6&7*";
    // Java实现随机数算法
     ...
     * @return
     */
    public static String generateRegistPwd(String mobile) {
        SecureRandom sr = null;
        try {
            sr = SecureRandom.getInstance(RANDOM_NUMBER_ALGORITHM);
            sr.setSeed(Base64.decodeBase64(SEED));
            byte[] buffer = sr.generateSeed(PWD_SIZE);
            Base32 codec = new Base32();
            byte[] bEncodedKey = codec.encode(buffer);
            String encodedKey = new String(bEncodedKey);
            return encodedKey.toLowerCase() + mobile.substring(7);
        } catch (NoSuchAlgorithmException e) {
            Sentry.captureException(e);
            // should never occur... configuration error
        }
...
//                hash = verify_code(decodedKey, t + i);
                hash = verify_code(decodedKey, t);// 即刻失效
            } catch (Exception e) {
...
//        System.out.println ( "qrCode="+qrCode+";key="+secret );
        ...
        String secret = "QU6NRWFTU6VZUXFE";
        String code = "381654";
        long time = System.currentTimeMillis();
        boolean result = checkCode(secret, code, time);
